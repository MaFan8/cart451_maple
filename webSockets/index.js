...
const wss = new WebSocket.Server({server}); // needs server inorder to run WS
...
}; // outside connection bc not reliant on it

//wss listens for the connection event for incoming sockets, and if one is connected -:
//ws is  a single socket instance  (can have many unique sockets for each client)
//req is the request
...
// when recieve a message, function will run
  ws.on('message', function incoming(message) {
    ...
    if (jsonParse.eventName === "stringLabel") {
        console.log("STRING");
        console.log(jsonParse.payload);
        // broadcast across all clients on network
    ...
    if (jsonParse.eventName === "jsonLabel") {
        ...
        // only send message to those who didn't send the message
        wss.clients.forEach(function each(client) {
